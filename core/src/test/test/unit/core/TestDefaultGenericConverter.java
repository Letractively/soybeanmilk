...
public class TestDefaultGenericConverter
{
	...
	public void getProperty() throws Exception
	{
		...
		{
			String id=converter.getProperty(bean, "id", null);
			Assert.assertEquals("111", id);
		}
		{
			Integer id=converter.getProperty(bean, "id", int.class);
			Assert.assertEquals(111, id.intValue());
		}
		{
			Integer size=converter.getProperty(bean, "size", null);
			Assert.assertEquals(7, size.intValue());
		}
		
		{
			String id=converter.getProperty(bean, "myBean2.id", null);
			Assert.assertEquals("222", id);
		}
		{
			Integer id=converter.getProperty(bean, "myBean2.id", int.class);
			Assert.assertEquals(222, id.intValue());
		}
		{
			Integer size=converter.getProperty(bean, "myBean2.size", null);
			Assert.assertEquals(8, size.intValue());
		}
		
		{
			Integer size=converter.getProperty(bean, "myBean2.myBean.myBean2.size", null);
			Assert.assertEquals(8, size.intValue());
		}
	}
